...
早期实现把平台管理员也挂在 `t-001` 下，于是同一个 `t-001` 既是「平台」又是「某个品牌客户」：

```mermaid
flowchart TB
    subgraph bad["早期实现（有矛盾）"]
...
**没有**：`factory:firmware:read`、`factory:batch:read`
...
7. ✅ **多实例部署已实测**（P11 收尾）：JWT 无状态，鉴权本身确实可水平扩展——
   2 实例 + 1 Nginx 轮询 **31 / 29**，WS 经 Nginx 握手正常（`11-测试与质量保障.md` 的 `## 8.3`）。
   ★ **但实测出两条硬约束**：① 共享 **SQLite** 卷时写吞吐**无提升**且报 `500`，**多实例必须配 PostgreSQL**；
   ② 多实例下的**限流与 WebSocket 会话路由仍缺外部状态支撑**（未实现，也未验证）。

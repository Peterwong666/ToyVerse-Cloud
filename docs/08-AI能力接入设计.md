...
|---|---|---|---|---|---|---|
| `device` 设备云 | 设备生成 / 激活 / 停用 / OTA | ✅ **已实现**（结果带 `simulated=True`） | ✅ 已实现骨架 | ✅ 已实现骨架 | — | — |
...
> 其中火山引擎的**签名算法已于 2026-09-24 落地实现**（AK/SK HMAC-SHA256，取自官方 demo 的
> `RtcApiRequester.py`，收口在 `VolcanoProvider._sign_request`）——详见
> [`13-火山引擎硬件对话智能体配置说明.md`](./13-火山引擎硬件对话智能体配置说明.md)
> 文末的取证边界表。**「签名已实现」不等于「联调已通过」，仍不要把「骨架」读成「可用」。**
>
> ★ 另：**设备端接入已于 2026-09-20 定案走「标准协议 WebSocket + 后端代注册」路线**
> （见 [`16-火山标准协议设备端接入方案与改动清单.md`](./16-火山标准协议设备端接入方案与改动清单.md)）；
> 本节的厂商 HTTP 适配器继续服务于**平台侧管理类调用**，两条路线并行不互斥。

> ★ **`mock` 声明了全部四类能力**（`ALL_CAPABILITIES`），包括 `device`。
> 设计理由是代码里的原话：**「`LOCAL`（mock）—— 四类能力全有，否则离线跑不通全流程」**
> （`backend/app/ai/base.py`）。因此 `MockProvider.provision_devices` / `activate_device` / `push_ota`
> 都会返回**模拟结果**，并且**每一个都带 `simulated=True`**——
> 这让调用方一眼能分辨「这是模拟通道的结果」，而不是真实厂商的返回（`ADR-07` 在离线场景下的落点）。
>
> 📌 **一处曾经写错的地方（已修正）**：本文档早期版本写的是「`mock` 不支持 `device`，设备生成由平台自身逻辑完成」。
> 这与实现不符——设备生成**确实**会经 `AIProvider`（4G 路线调厂商的 `provision_devices`），
> 而离线引擎在这个环节给出的是带 `simulated=True` 的模拟结果。
> 发现方式：写 P11-B 学习手册时按代码核对能力矩阵，发现文档与 `mock/engine.py` 的 `capabilities = ALL_CAPABILITIES` 冲突。
...
> 便于联调时集中替换。火山引擎的签名**已于 2026-09-24 实现真实算法**（`VolcanoProvider._sign_request`）。
...
| **签名算法与签名位置** | ✅ **已实现（2026-09-24）**：AK/SK HMAC-SHA256，取自官方 demo 的 `RtcApiRequester.py`，收口在 `VolcanoProvider._sign_request` |
| 部分 Body 字段名 | ⚠️ 按官方 PascalCase 惯例统一，**未逐字核对**（风险示例：`AibotId` vs `AgentId`） |
| `TTSConfig` / `LLMConfig` 层级 | ⚠️ 同构推断（仅 `ASRConfig` 逐字来自官方示例） |
| 真实调用 | ⚠️ **从未发生**（无密钥） |

> **签名逻辑收口在 `VolcanoProvider._sign_request` 一处**（2026-09-24 起为真实实现；
> 签名需要请求体参与计算，因此 `_call_action` 重写了请求逻辑、复用基类的重试与错误处理）。
...
2. **火山引擎签名算法已实现（2026-09-24），但部分 Body 字段名仍未逐字核对**（`TTSConfig` / `LLMConfig` 层级为同构推断），且真实 Action 调用从未发生。

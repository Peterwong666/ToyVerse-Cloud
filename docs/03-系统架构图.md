...
| **密钥只进不出** | 落库加密（Fernet）+ 响应只回掩码 | 早期实现把 `SecretKey` 明文下发到前端 |
...
1. **压测只有一轮短测**（P11 收尾实测）：20 并发 × 15 秒，读 **73.6 QPS**（SQLite）/ **72.0**（PG），
   写 **30.7**（SQLite）/ **93.0**（PG）。✅ 比「完全没有数字」前进了一步，
   但 ⚠️ **没有劣化曲线、没有参数扫描**（连接池 / `--workers`），
   因此本文对「单实例 4 worker 长期能扛多少」**仍不能给结论**。
   → 详见 `11-测试与质量保障.md` 的 `## 8.1`。
2. ✅ **多实例部署已实测**（2 实例 + 1 Nginx）：轮询 **31 / 29**，WS 经 Nginx 正常。
   ★ **但实测给出了反面结论**：两实例**共享同一 SQLite 卷**时写吞吐**无提升**（31.4 QPS）且出现 **2×500**。
   → **多实例必须配 PostgreSQL**；SQLite 只适合单实例。
   ⚠️ 多实例下 WebSocket 会话路由、限流、`outbox_events` 投递**仍缺外部状态支撑**，未验证。
3. ✅ **Nginx TLS 已实测**（自签证书）：HTTPS API/SPA 全 200、HTTP→HTTPS **301**、
   `wss://` 握手后以 **4401** 关闭、`TLSv1.3 / AES-256-GCM-SHA384`。
   ⚠️ 但**仓库未附证书**，`deploy/nginx.conf` 的 80→443 段仍是注释。
4. **PostgreSQL 已真跑验证**：起真实 PG 16 → 迁移到 `0015` → 种子 → 应用 healthy → 冒烟 **57/57**。
   ✅ **并已完成并发写压测**：写 **93.0 QPS / 0 失败**（SQLite 对照 30.7 QPS / 4×500）。
   ⚠️ 但**连接池参数未调优**，「多实例 + PG」组合**未实测**。

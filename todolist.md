...
| # | 待办 | 处理结果 |
|---|---|---|
| 1 | CI 从未在真实 GitHub 上跑过 | ✅ **把能本地验的都验了**：YAML 可解析（5 job）、`run` 引用的脚本/路径全部存在、命令与 Makefile 目标逐字一致；★ **并修掉一处真实冗余**——`pytest -q` 本就会收集 e2e，之后又单独跑一次，**每次 CI 白等约 5 分钟**（改为 `--ignore=tests/e2e`）。✅ **已在真实 GitHub 跑通**（2026-09-17，run #1 五 job 全绿）——见下「推送与 CI 首次实跑」 |
...
- **仓库**：`https://github.com/Peterwong666/ToyVerse-Cloud`（public）。
- **推送内容**：`main` 分支 + 附注 tag `v1.0.0`。
  ★ 远端原有一个**较旧的快照**（20 提交 / 280 文件，来自「账号中立化」那次），
  与本地的历史**无共同祖先**（本机做过作者重写，全部 hash 变了），因此是**强推**；
  旧历史已在本地留为分支 `backup/origin-main-b6c1f06` 以防万一。
  推送前用 `--dry-run` 演练过，并核对「远端独有文件」只有 `SUMMARY.md`（用户已决定不保留）。
- ★★ **首次真跑抓到 CI 的一个真缺陷**：`ci.yml` 的 `on.push.branches` 是 **`master`**，
  而默认分支是 **`main`** → **推送不触发任何 CI**。发现方式：查 API 得 `total_count = 0`。
  修复（改为 `main`）后重推，**run #1 五 job 全绿**。
- **run #1 结果**（commit `8aeea28`）：代码规范 43s ✅ / 测试（pytest）2m42s ✅ /
  契约一致性 25s ✅ / 前端导入契约 5s ✅ / 敏感信息扫描 6s ✅ —— **无步骤被跳过**。
- **独立复核**：另把仓库 `git clone` 到空目录（**无 `.env`、无 `venv`**，与 CI 条件一致），
  逐条重跑 CI 的 5 条命令 → 全部通过（`813 passed`、覆盖率 **69.92%** ≥ 65 门槛）。

**剩余待办（诚实记录）**：① **多实例 + PostgreSQL 组合未验证**（只证明了「SQLite 多实例不行」，反向未验）；
② 真实厂商联调与儿童数据合规未实现；③ 截图仍由 headless Chrome 采集；
④ 压测只有一轮短测（无劣化曲线、未调连接池、证书为自签）；
⑤ CI 不构建镜像、未接入依赖漏洞扫描，且 CI 环境与本机不同（本机实测数字仍以本机为准）。

...
| 5 | 覆盖率 70%、无压测 | ✅ CI 新增**覆盖率下限 `--cov-fail-under=65`**（实测 70，留 5 点余量）——本项目到 P11 才发现「804 用例全绿但覆盖率只有 70%」，**当时没有任何机制会提醒**；⚠️ **压测仍无法做**（需真实负载模型），只能如实声明 |
| 6 | PostgreSQL / Nginx TLS / 多实例仅部分验证 | ✅ **PostgreSQL 已真跑**（`--profile postgres` 起 PG 16 → 迁移到 `0015` → 种子 → 应用 healthy → **冒烟 57/57 全通过**）★ **并因此抓到一个真实缺陷**（见下）。⚠️ **TLS 与多实例仍未做**：TLS 需证书、多实例需外部状态（限流 / WS 会话路由），均如实登记 |
...
**剩余待办（诚实记录）**：① CI 首次推送后必须实跑（本机无 remote）；② Nginx TLS 与多实例未验证；
③ 压测未做；④ 真实厂商联调与儿童数据合规未实现；⑤ 截图仍由 headless Chrome 采集。

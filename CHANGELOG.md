...
> **814 passed**），并完成 **P11-D 收官验证**——负载测试、Nginx TLS、多实例三项**已实测**，
> 且**已在 GitHub 上跑通 CI**（run #1 五 job 全绿）。
> ⚠️ **本版本已知的边界**：AI 与设备链路未与真实厂商联调、未实现儿童数据合规（PIPL/COPPA）、
> 代码覆盖率 70%、压测只有一轮单机短测（**不能作为容量规划依据**）
> —— 详见下方「计划中」与各文档的「已知局限」小节。
...
- **`scripts/scan_secrets.py`**：敏感信息扫描四类规则 —— 弱口令 / 硬编码密钥 / **出参模型里的明文密钥字段** / 敏感文件入库（`.env`、`data/`、`learning/`）；扫描被跟踪文件 **0 命中**（当时 236 个；P11 新增截图与测试后为 247 个），并带一个「规则确能命中」的自测
...
**P11-D 收官验证（把三条 ⚠️ 局限真的跑掉）**

- **`scripts/load_test.py`** —— 基于标准库 + `httpx` 的 asyncio 负载测试器，
  **不引入 `ab` / `wrk` / `hey`** 等外部依赖（本机没有，也不想为此加一条部署前置）。
  带两个守卫：非 GET 必须显式 `--allow-write`；只读方法被传 `--body` 时**直接拒绝执行**
  （防止「忘了加 `--method`」把压测打成写库）
- **`backend/tests/integration/test_seed_admin_users.py`** —— 初始账号播种的幂等性回归守卫（**5 条**）：
  新建 / 重跑空操作（**保密码**）/ **改名同步** / 改名后第三次仍幂等 / 名字被别的 `id` 占用则跳过
- `docs/11` 新增 **`## 8. 性能与部署验证`**（8.1 压测 / 8.2 TLS / 8.3 多实例 / 8.4 播种缺陷 /
  **8.5 仍未证明的事**）
...
- ★ **初始账号播种不幂等：改一个 `.env` 里的账号名，已有库的部署就无限重启**
  - 根因：`_seed_admin_users` **按 `account` 查重、却按固定 `id` 插入**，两个判据不一致。
    改账号名后按新名字查不到 → 拿同一个 `id` 再插一次 → `IntegrityError: UNIQUE constraint failed: user_accounts.id`；
    容器入口是「**先播种、再起服务**」，于是表现为**启动即崩、无限重启**
  - 修复：查重一律按 **`id`**；三条路径都覆盖——同名 → 空操作（**不动密码**）、
    改名 → **同步登录名并告警**、目标名已被**别的 `id`** 占用 → **跳过并告警**（否则会撞唯一键）
  - 验证：在**真实故障卷**上实测（`Restarting (1)` → `healthy`、日志出现改名告警、
    `password_hash` 未变、重启幂等）+ 5 条回归测试（**改名那条在修复前的实现上会失败**，已实测）
  - ★ 发现方式：**P11-D 的多实例实验顺带炸出来的**——新库 / 测试库 / CI **全都复现不出来**

### 变更（P11-D：把三条 ⚠️ 局限改为实测）

以下三项此前在 `docs/09` 与 `docs/11` 里登记为「**未验证**」，本次**各真跑了一遍**：

- **负载测试（20 并发 × 15 秒，同一脚本打 SQLite 与 PostgreSQL）**：读 **73.6 / 72.0** QPS；
  写 **30.7 / 93.0** QPS；SQLite 写失败 **4/487**（`database is locked` → `500`），PG **0/1414**。
  ★ **修正了本项目自己的假设**：**读路径上 SQLite 并不输**（吞吐持平，尾延迟 p95 436 vs 994 ms 还更好），
  PostgreSQL 的优势**完全集中在写路径**（×3 吞吐、失败归零、p95 2.9 s → 0.6 s）。
  → 选型的准确表述是「**只有需要并发写时才必须上 PostgreSQL**」，而不是含混的「PG 什么都更快」
- **Nginx TLS（自签证书）四项全过**：HTTPS 提供 API `200` / SPA 与静态资源全 `200` /
  HTTP→HTTPS **`301`** / ★ **`wss://` 握手成功并以 close code `4401` 关闭**（期望值）；
  `openssl s_client` 佐证 `New, TLSv1.3, Cipher is TLS_AES_256_GCM_SHA384`（没有静默回退明文）
- **多实例（2 实例 + 1 Nginx 轮询）**：请求分发 **31 / 29**、WS 经 Nginx → `4401`；
  ★ **共享同一 SQLite 卷并发写：31.4 QPS + 2×500**（单实例 30.7 → **无提升**，冲突翻倍）
  → **加实例救不了 SQLite 的写并发；多实例必须配 PostgreSQL**
- 文档同步：`docs/09` 的 `## 2. 性能 SLO` 重写为实测基线（`2.1` 并发基线 / `2.2` 业务阈值）；
  `docs/11` 新增 `## 8`；`docs/01` / `docs/03` / `docs/07` / `docs/10` / `docs/12` / `README.md`
  的「未验证」条目同步为实测结论
- 测试计数：**814 passed**（553 个测试函数 / 26 个文件）
- ★★ **已推送到 GitHub 并在真实环境跑通 CI**（`https://github.com/Peterwong666/ToyVerse-Cloud`，
  推送 `main` + 附注 tag `v1.0.0`）。★ **首次真跑抓到 CI 的一个真缺陷**：
  `ci.yml` 的 `on.push.branches` 写的是 **`master`**，而默认分支是 **`main`** →
  **推送根本不触发任何 CI**（查 API 得 `total_count = 0`）；修复后 **run #1 五 job 全绿**
  （代码规范 43 s / 测试 2 m 42 s / 契约 25 s / 前端导入 5 s / 敏感扫描 6 s，**无步骤被跳过**）。
  ★ 这条与 P10 的 12 个问题同类：**「配置存在」不等于「配置生效」**。
- **全仓库表述中性化**：把指向早期实现的称谓统一为「**早期实现 / 早期第三方实现**」，
  并把若干历史机制名改为中性词。保留第三方 SDK / 库的 9 处合法用法（指代的是那些库自身）；
  `learning/` 因不上传不在范围内
- ★ 中性化的校验踩坑：`git ls-files` 默认会把 CJK 文件名转义（`\345...`），
  `grep` 因此**静默跳过整个 `docs/`** —— 必须用 `git -c core.quotePath=false ls-files`，
  否则会把「没检查」误判成「已干净」
...
- 新增 `docs/05-数据模型与ER图.md`（**1809 行**）：8 张分域 ER 图 / **46 张表 × 655 个字段的完整字段字典**（★ **由 ORM 元数据程序化导出**，非手抄）/ 六条复合唯一键的业务含义（含「指标表唯一键里为什么必须有 `client_product_id`」）/ 20 条复合索引的服务对象 / 全量状态枚举 / ★ **`in_stock` 语义对齐**（订单维度 vs 设备维度、与早期实现的差异、以及一条自检规则）
...
- **P11-C 待办（剩余的诚实局限，4 条）**：① **截图由独立 headless Chrome 采集**（`bsk screenshot` 在本机只能成功一次，之后持续 RPC 超时）——是真实服务渲染的真实页面，但采集通道不是用户日常浏览器；② **截图是静态画面**，不含弹窗 / 下拉 / Toast 等交互过程，也不含响应式适配；③ **陌生环境验证的 72 秒偏乐观**（本机 pip 有缓存，冷机器会更久），且**未在真正的另一台机器上验证**（同机不同目录）；④ ✅ **已解决**：`CHANGELOG.md` 已切 `[1.0.0]`，并建本地 git tag；⑤ ✅ **已解决**：`docs/12` 的过期表述已同步；⑥ ✅ **已解决**：协作用语统一为「AI Agent」，且全仓库中指向早期实现的表述已统一改为中性口径
...
- **P11-A 待办（文档侧的诚实局限，7 条）**：① ✅ **已解决**（P11-C）：`docs/assets/` 已有 14 张各端截图（⚠️ 由无头浏览器采集、静态画面、不含交互过程）；② `docs/04` PRD 是**事后补写**，无用户调研与需求优先级；③ `docs/10` 的基准数据来自**离线模拟引擎**，不代表真实厂商链路；④ **门禁只保证「名字与数字对得上」**，叙述性描述无法校验；⑤ `docs/05` 的字段类型未逐列核对 SQLite 实际 DDL；⑥ `scripts/check_docs.py` **自身没有单元测试**；⑦ `docs/03`/`docs/12` 的分层与决策解释属事后归纳
- **P11-D 待办（收官验证侧仍未证明的事，5 条）**：① ✅ **已解决**：多实例 + PostgreSQL 组合已验证（读 142.5 QPS / 写 66.2 QPS / 0 失败），详见 `[未发布]`；② 压测**只有一轮单机短测**（20 并发 × 15 秒），**无劣化曲线**（内存增长、连接泄漏、慢查询累积均未观测），**不能作为容量规划依据**；③ **未做参数扫描**（PG 连接池、`--workers` 取值）；④ **WebSocket 长连接容量未测**（只验证了握手与关闭码，未压「同时保持 N 条对话流」）；⑤ **写路径只压了 1 个轻量端点**（`simulate-heartbeat`），批量导入 / 分配执行等重写未压；⑥ 证书是**自签**的（`CN=localhost`），仓库未附证书，默认配置仍是 HTTP
- **P10 待办（部署侧的诚实局限，7 条）**：① ✅ **已解决**（P11-D）：**CI 已在真实 GitHub 跑通**（2026-09-17，`Peterwong666/ToyVerse-Cloud` run #1，五 job 全绿）；★ 首跑即抓到「`ci.yml` 的触发分支写成 `master`、而默认分支是 `main` → **推送不触发任何 CI**」，已修复；② **CI 不构建镜像**（pip 全量安装是分钟级，理由写在 `ci.yml` 注释里）；③ ✅ **已解决**（P11-C/P11-D）：PG 实例已真跑（迁移 `0015` + 种子 + healthy + 冒烟 57/57），**并做了并发写压测**（写 93.0 QPS / 0 失败）；⚠️ 连接池参数仍未调优；④ ✅ **已解决**（P11-D/P11-E）：TLS、多实例 SQLite、**多实例 + PostgreSQL 组合均已实测**；⚠️ 证书为**自签**；⑤ **`scripts/reset_db.sh` 是同盘备份**，宿主机磁盘故障时备份与数据一起丢；⑥ ✅ **已量化**（P11-D）：SQLite 20 并发写同一行 → **30.7 QPS、4/487 报 500**（`database is locked`），PG **93.0 QPS、0 失败**；且**加实例无提升**（双实例 31.4 QPS）→ **多实例必须配 PostgreSQL**；⑦ **冒烟脚本只覆盖读路径 +2 个幂等写**，写路径验证依赖 `make test`（隔离测试库）
...
**测试规模**：后端 118 → **814** 条（P3 82；P7 49；P4 146；P5 53；P6 281；P8 20 + 全闭环 e2e 1；**P9 46**：租户隔离 26 + 指标 9 + OTA 14）。**P10 未新增单元测试**，新增的是部署期验证：全端点冒烟 57 项、持久化一致性检查、敏感信息扫描 236 文件 0 命中、经 Nginx 的 WebSocket 握手。**P11 新增 10 个用例**（两组各 5 个）：
① `test_sql_dialect_portability.py`（3 个函数 / 5 个用例）守卫「日期必须绑 `date` 而不是 `str`」这条**在 SQLite 上复现不出来**的 PG 专有约束（详见 `## 7.5`）；
② `test_seed_admin_users.py`（5 个函数）守卫初始账号播种的幂等性——**这是 P11-D 多实例实验炸出的真缺陷的回归守卫**（改名路径在修复前的实现上会失败）。
其余 P11 产出是文档一致性门禁（八类校验）、文档集（14 篇）、学习手册（20 文件 / 8177 行，不入库）、14 张各端截图、README 终版与负载测试脚本——**文档与部署验证阶段靠「一致性门禁 + 真跑一次」而不是靠测试**。

**两个计数口径**：**553 个测试函数**（`def test_` 计数，26 个文件）/ **814 个用例**（pytest 实际执行数）。差额来自 `@pytest.mark.parametrize` 展开（例：租户隔离矩阵把「端点 × 越权形态」展开）。

**覆盖率**：**70%**（10501 语句 / 3141 未覆盖）。`ai_config_service` 29% / `metrics_service` 29% / `ota_service` 30% 是三个最低点——★ **「814 passed」不等于「测试充分」**，详情与根因见 `docs/11`。

**验收强度（本项目的固定做法）**：每个阶段除自动化测试外，都要在**真实服务**上跑一遍端到端接口验收，并用 `browser-skill` 驱动真实 Chromium 点一遍关键路径。累计：P8 接口验收 50/50 + 浏览器 11 项；P9 浏览器 6 项；**P10 冒烟 57/57（直连与经 Nginx 各一次）+ 容器 healthy + 起停数据一致**；**P11-A `make check` 全绿（804 passed in 310.55s / mypy 95 文件 / 契约一致 / 文档一致性通过）**；**P11-C 陌生环境按 README 从克隆到四端可用 ≈91 秒**；**P11-D `make check` 全绿（814 passed in 256.49s）+ 负载测试 / TLS / 多实例三项实测**。
...
- 本项目由三个遗留项目整合而成，整合策略为「各取所长」：
  ...
  - 早期实现的运行时快照提供数据库 Schema 结构与多租户 RBAC 设计

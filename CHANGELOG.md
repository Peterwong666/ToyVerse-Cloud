...
- P8 终端用户小程序端：扫码解析 `JX|` / `JD|` 双分支、4G 与 Wi-Fi 两条激活路径、WebSocket 流式对话（含 SSE 降级）、4G 充值、小程序 9 屏
- P9 AI 配置与运营看板：知识库、音色、内容安全、指标聚合（**按 `product_id` 隔离**，修复 P-07 / P-08）
...
**迁移编号调整**：P1 落地时实际占用了 `0001`–`0004`（与原始计划的 0001/0002 不同），因此目录域顺延为 `0005`/`0006`，P7 使用 `0007`；P4 起顺延为 `0008`–`0010`（订单 / 设备 / 批次），P5 为 `0011`，**P6 为 `0012`（工厂账号归属）+ `0013`（设备工单归属），P9 顺延为 `0014`**。

**测试规模**：后端 118 → **737** 条（P3 82；P7 49；P4 146；P5 53；**P6 281**：脱敏 132、工厂流程 42、脱敏红线 7，以及补齐 P5 欠账的分配 33 / 租户隔离 36 / 心跳 31）。

**P6 的验收强度**：除 737 条自动化测试外，另做**真实服务 + 真实 HTTP 的端到端接口验收 71/71**（派单 → 烧录 → 抽检 → 出货 → 入库 → 分配 → 冻结，含脱敏红线、跨厂隔离、权限边界）与**浏览器实测 12 项**。本轮 3 个真缺陷**全部由这两轮实测发现**，代理自测的 275 条全绿并没能覆盖它们 —— 这是本项目坚持「代理自测全绿 ≠ 功能可用」的直接证据。

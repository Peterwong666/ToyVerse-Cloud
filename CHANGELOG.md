...
- **P11-D 待办（收官验证侧仍未证明的事，6 条）**：① **多实例 + PostgreSQL 组合未测**——只证明了「SQLite 多实例不行」，**反向未验**；② 压测**只有一轮单机短测**（20 并发 × 15 秒），**无劣化曲线**（内存增长、连接泄漏、慢查询累积均未观测），**不能作为容量规划依据**；③ **未做参数扫描**（PG 连接池、`--workers` 取值）；④ **WebSocket 长连接容量未测**（只验证了握手与关闭码，未压「同时保持 N 条对话流」）；⑤ **写路径只压了 1 个轻量端点**（`simulate-heartbeat`），批量导入 / 分配执行等重写未压；⑥ 证书是**自签**的（`CN=localhost`），仓库未附证书，默认配置仍是 HTTP
- **P10 待办（部署侧的诚实局限，7 条）**：① ✅ **已解决**（P11-D）：**CI 已在真实 GitHub 跑通**（2026-09-17，`Peterwong666/ToyVerse-Cloud` run #1，五 job 全绿）；★ 首跑即抓到「`ci.yml` 的触发分支写成 `master`、而默认分支是 `main` → **推送不触发任何 CI**」，已修复；② **CI 不构建镜像**（pip 全量安装是分钟级，理由写在 `ci.yml` 注释里）；③ ✅ **已解决**（P11-C/P11-D）：PG 实例已真跑（迁移 `0015` + 种子 + healthy + 冒烟 57/57），**并做了并发写压测**（写 93.0 QPS / 0 失败）；⚠️ 连接池参数仍未调优；④ ✅ **已解决**（P11-D）：TLS 与多实例均已实测（TLS 四项全过、多实例轮询 31/29）；⚠️ 证书为**自签**、且**「多实例 + PostgreSQL」组合未验**；⑤ **`scripts/reset_db.sh` 是同盘备份**，宿主机磁盘故障时备份与数据一起丢；⑥ ✅ **已量化**（P11-D）：SQLite 20 并发写同一行 → **30.7 QPS、4/487 报 500**（`database is locked`），PG **93.0 QPS、0 失败**；且**加实例无提升**（双实例 31.4 QPS）→ **多实例必须配 PostgreSQL**；⑦ **冒烟脚本只覆盖读路径 +2 个幂等写**，写路径验证依赖 `make test`（隔离测试库）

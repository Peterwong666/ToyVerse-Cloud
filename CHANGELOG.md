...
- 新增 `docs/04-产品需求文档PRD.md`：产品定位与**非目标** / 6 角色 + 终端用户 / **27 条术语表** / 页面清单（含 **18 个未实现菜单项**的明示）/ 三处刻意能力收窄的说明 / **7 条界面设计原则**（每条都有真实依据）/ 非功能需求 / 版本变更。★ 明确标注「**本 PRD 为事后补写**」
...
- **P11-B 学习手册**（`learning/`，不上传 GitHub）：`README` + `00`–`12` 共 13 个模块 + 6 个练习，每个模块要求 12 段结构（学习目标 / 核心概念 / 本项目中的体现 / 分步讲解 / 重点标注 / 动手练习 / 自测题附答案 / 常见错误 / 与前后端沟通要点 / 检查清单 / 拓展知识）
- **P11-C 收尾**：`README.md` 终版、各端关键页面截图、`CHANGELOG.md` 完整版本历史、最终验证（`learning/` 未被 git 跟踪、陌生环境 5 分钟可跑起来）
...
**测试规模**：后端 118 → **804** 条（P3 82；P7 49；P4 146；P5 53；P6 281；P8 20 + 全闭环 e2e 1；**P9 46**：租户隔离 26 + 指标 9 + OTA 14）。**P10 未新增单元测试**，新增的是部署期验证：全端点冒烟 57 项、持久化一致性检查、敏感信息扫描 236 文件 0 命中、经 Nginx 的 WebSocket 握手。**P11-A 未新增单元测试**，新增的是文档一致性门禁（8 类校验）与文档集本身（14 篇）。

**两个计数口径**：**545 个测试函数**（`def test_` 计数）/ **804 个用例**（pytest 实际执行数）。差额来自 `@pytest.mark.parametrize` 展开（例：租户隔离矩阵把「端点 × 越权形态」展开）。
...
**验收强度（本项目的固定做法）**：每个阶段除自动化测试外，都要在**真实服务**上跑一遍端到端接口验收，并用 `browser-skill` 驱动真实 Chromium 点一遍关键路径。累计：P8 接口验收 50/50 + 浏览器 11 项；P9 浏览器 6 项；**P10 冒烟 57/57（直连与经 Nginx 各一次）+ 容器 healthy + 起停数据一致**；**P11-A `make check` 全绿（804 passed in 310.55s / mypy 95 文件 / 契约一致 / 文档一致性 8 类通过）**。

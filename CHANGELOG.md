...
- P9 AI 配置与运营看板：知识库、音色、内容安全三开关的**商户端配置面板**、按 `product_id` 隔离的运营指标聚合（修复 P-07 / P-08）、OTA 推送（迁移编号 **`0015`**）
...
**迁移编号调整**：P1 落地时实际占用了 `0001`–`0004`（与原始计划的 0001/0002 不同），因此目录域顺延为 `0005`/`0006`，P7 使用 `0007`；P4 起顺延为 `0008`–`0010`（订单 / 设备 / 批次），P5 为 `0011`，P6 为 `0012`（工厂账号归属）+ `0013`（设备工单归属），**P8 为 `0014`（终端用户与充值，原属 P9），P9 顺延为 `0015`**。

**测试规模**：后端 118 → **758** 条（P3 82；P7 49；P4 146；P5 53；P6 281；**P8 20 + 全闭环 e2e 1**）。
